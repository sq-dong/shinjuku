...
struct ThreadContext {
    int id;
    int lcore_id;
    DpdkBimodalLatencyClient* client;
    double qps;
    bool is_receiver;
    bool drain_recv;
...
void send_thread_func(ThreadContext* ctx) {
    ...
    DpdkBimodalLatencyClient* client = ctx->client;
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 6) {
        std::cerr << "Usage: " << argv[0] << " <server_port> <qps> <work1_ns> <work2_ns> <ratio>"
                  << " <send_threads> [recv_threads] [drain_recv] [output_file_or_not]" << std::endl;
        std::cerr << "Example: " << argv[0] << " 1234 1000000 1000 10000 0.9 4 4 0 1" << std::endl;
    ...
    uint64_t work1 = static_cast<uint64_t>(atoi(argv[3]));
    uint64_t work2 = static_cast<uint64_t>(atoi(argv[4]));
    double ratio = atof(argv[5]);
    int num_send_threads = atoi(argv[6]);
    int num_recv_threads = (argc >= 8) ? atoi(argv[7]) : 1;
    int drain_recv = 0;
    int output_file_or_not = 0;
    if (argc >= 10) {
        drain_recv = atoi(argv[8]);
        output_file_or_not = atoi(argv[9]);
    } else if (argc >= 9) {
        drain_recv = atoi(argv[8]);
    }
    ...
    if (num_recv_threads > num_send_threads) {
        num_recv_threads = num_send_threads;
        std::cout << "[INFO] Limiting receive threads to " << num_recv_threads
                  << " (one per send thread)" << std::endl;
    }

    signal(SIGINT, sigint_handler);

    std::cout << "\n=================================================================" << std::endl;
    std::cout << "DPDK Latency Client - Multi-Threaded Version (Bimodal)" << std::endl;
    std::cout << "=================================================================" << std::endl;
    ...
    std::cout << " Bimodal work:    work1=" << work1 << " ns work2=" << work2 << " ns ratio=" << ratio << std::endl;
    std::cout << " Send threads:    " << num_send_threads << std::endl;
    std::cout << " Receive threads: " << num_recv_threads << " (one per send thread)" << std::endl;
    std::cout << "\nPress Ctrl+C to stop the test" << std::endl;
    std::cout << "=================================================================\n" << std::endl;

    DpdkPortManager* port_mgr = DpdkPortManager::getInstance();
    if (!port_mgr->initialize(num_send_threads, num_send_threads)) {
        std::cerr << "Failed to initialize DPDK port with multiple queues" << std::endl;
        return -1;
    }

    std::vector<DpdkBimodalLatencyClient*> clients;
    ...
    for (int i = 0; i < num_send_threads; ++i) {
        ...
        auto* send_client = new DpdkBimodalLatencyClient(server_port, qps / num_send_threads,
                                                        work1, work2, ratio, i, client_port, "");
        ...
        if (i < num_recv_threads) {
            auto* recv_client = new DpdkBimodalLatencyClient(server_port, 0, work1, work2, ratio,
                                                            i, client_port, "");
            if (!recv_client->initialize()) {
                std::cerr << "Failed to initialize receiver client for sender " << i << std::endl;
                delete recv_client;
                continue;
            }
            ...
            recv_ctx->id = i + num_send_threads;
            recv_ctx->lcore_id = num_send_threads + i;
    ...
    while (!should_stop.load()) {
        ...
        for (auto client : clients) {
            total_sent += client->getPacketsSent();
            total_received += client->getPacketsReceived();
        }
        std::cout << "\rProgress: Sent=" << total_sent
                  << " Received=" << total_received
                  << " Loss=" << std::fixed << std::setprecision(2)
                  << (total_sent > 0 ?
                      (100.0 * (total_sent - total_received) / total_sent) : 0.0)
                  << "%" << std::flush;
    }
    ...
    if (drain_recv) {
        for (int i = 0; i < num_send_threads; i++)
            if (threads[i * 2].joinable())
                threads[i * 2].join();
        for (int i = 0; i < num_recv_threads; i++)
            if (threads[i * 2 + 1].joinable())
                threads[i * 2 + 1].join();
    } else {
        for (auto& t : threads) {
            if (t.joinable())
    ...
    for (int i = 0; i < num_send_threads; ++i) {
        int recv_idx = 2 * i + 1;
        if (recv_idx < (int)clients.size()) {
            auto s = clients[recv_idx]->getLatencySamples();
            all_latency_samples.insert(all_latency_samples.end(), s.begin(), s.end());
            auto nc = clients[recv_idx]->getNetworkerCySamples();
            all_networker_cy.insert(all_networker_cy.end(), nc.begin(), nc.end());
            auto dc = clients[recv_idx]->getDispatcherCySamples();
    ...
    if (output_file_or_not == 1) {
        ...
        if (out_file.is_open()) {
            ...
            out_file << "DPDK Bimodal Latency Client Statistics\n";
            out_file << "======================================\n\n";
